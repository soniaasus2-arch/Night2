...
-- ============================================================
-- SISTEMA DE NOTIFICAÇÃO (LARRY, MUTANT, WORKER, WORKERHEAD)
-- ============================================================

local notifierActive = false
local notifierLoop = nil
...
local function showNotification(text, cor)
    ...
    frame.BackgroundColor3 = cor or Color3.fromRGB(0, 0, 0)
...
local function iniciarNotificador()
    if notifierLoop then
        notifierLoop:Disconnect()
        notifierLoop = nil
    end
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Model") then
            if obj.Name == "Larry" then
                showNotification("👾 LARRY ESTÁ AQUI!", Color3.fromRGB(200, 50, 50))
            elseif obj.Name == "Mutant" then
                showNotification("🧟 MUTANT ESTÁ AQUI!", Color3.fromRGB(150, 50, 200))
            elseif obj.Name == "Worker" then
                showNotification("👷 WORKER ESTÁ AQUI!", Color3.fromRGB(50, 150, 200))
            elseif obj.Name == "WorkerHead" then
                showNotification("👷 WORKERHEAD ESTÁ AQUI!", Color3.fromRGB(50, 200, 150))
            end
        end
    end
...
addButton(geralCard, "Ativar Notificador", function()
    iniciarNotificador()
end)

addButton(geralCard, "Desativar Notificador", function()
